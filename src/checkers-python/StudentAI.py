...
class StudentAI():
    ...
    def minimax_move(self, moves: [list]):
        ...
        for chess in moves:
            for move in chess:
                val = self.min_value(move, self.depth, max_value, math.inf)
                if val > max_value:
                    best = [move]
                    max_value = val
                elif val == max_value:
    ...
    def min_value(self, move, depth, alpha, beta):
        ...
        moves = [m for chess in self.board.get_all_possible_moves(self.opponent[self.color]) for m in chess]
    ...
    def max_value(self, move, depth, alpha, beta):
        ...
        moves = [m for chess in self.board.get_all_possible_moves(self.color) for m in chess]
        # moves = self.board.get_all_possible_moves(self.color)
    ...
    #     return board.black_count - board.white_count if self.color == 1 \
    #             else board.white_count - board.black_count
    def utility(self, board, color):
